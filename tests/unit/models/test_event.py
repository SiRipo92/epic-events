...
class TestEventDurationHours:
    ...
    Uses parametrize to cover all duration scenarios in one sweep.
    Each tuple is (start, end, expected_hours, description).
    """

    @pytest.mark.parametrize("start, end, expected", [
        (datetime(2025, 9, 1,  9,  0), datetime(2025, 9, 1, 17,  0), 8.0),   # happy: 8hr day
        (datetime(2025, 9, 1, 18,  0), datetime(2025, 9, 1, 19, 30), 1.5),   # edge: 90 minutes
        (datetime(2025, 9, 1, 22,  0), datetime(2025, 9, 2,  2,  0), 4.0),   # edge: crosses midnight
        (datetime(2025, 9, 1, 12,  0), datetime(2025, 9, 1, 12,  0), 0.0),   # edge: zero duration
    ])
